  ...
  when_system_active_and_actuator_unconfigured_expect_system_passing)
{
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 1.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 101.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 102.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 202.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(empty_keys, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 203.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);
  EXPECT_FALSE(rm_->perform_command_mode_switch(empty_keys, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 303.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 403.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 503.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 603.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);
};
  ...
  ResourceManagerPreparePerformTest, when_system_active_and_actuator_inactive_expect_both_passing)
{
  ...
  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_system, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 1.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 1.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 101.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 101.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 102.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 102.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 202.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 202.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(empty_keys, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 203.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 203.0);
  EXPECT_FALSE(rm_->perform_command_mode_switch(empty_keys, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 303.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 303.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 304.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 304.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_actuator, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 404.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 404.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_actuator, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 405.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 405.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_actuator, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 505.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 505.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(empty_keys, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 506.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 506.0);
  EXPECT_FALSE(rm_->perform_command_mode_switch(empty_keys, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 606.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 606.0);
};
  ...
  when_system_unconfigured_and_actuator_active_expect_actuator_passing)
{
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 0.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, legal_keys_system));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 100.0);
  ...
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_system, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 200.0);
  ...
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 300.0);

  // When TestActuatorHardware is INACTIVE expect OK
  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_actuator, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 301.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_actuator, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 401.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(legal_keys_actuator, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 402.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(legal_keys_actuator, empty_keys));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 502.0);

  EXPECT_TRUE(rm_->prepare_command_mode_switch(empty_keys, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 503.0);
  EXPECT_TRUE(rm_->perform_command_mode_switch(empty_keys, legal_keys_actuator));
  EXPECT_EQ(claimed_system_acceleration_state_->get_value<double>(), 0.0);
  EXPECT_EQ(claimed_actuator_position_state_->get_value<double>(), 603.0);
};
