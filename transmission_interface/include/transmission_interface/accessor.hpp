...
template <typename T>
std::string to_string(const std::vector<std::shared_ptr<T>> & list)
{
  std::stringstream ss;
  ss << "[";
  for (const auto & elem : list)
...
template <class T>
std::vector<std::string> get_names(const std::vector<std::shared_ptr<T>> & handles)
{
  std::set<std::string> names;
  std::transform(
    handles.cbegin(), handles.cend(), std::inserter(names, names.end()),
...
template <typename T>
std::vector<std::shared_ptr<T>> get_ordered_handles(
  const std::vector<std::shared_ptr<T>> & unordered_handles, const std::vector<std::string> & names,
